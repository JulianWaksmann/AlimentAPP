...
import { GetPedidosAsignados } from '@/app/api/pedidosVenta';
import Header from '@/app/components/Header';
import { PedidosAsignadosResponse } from '../pedidos-asignados/page';


const PedidosDespachadosPage = () => {
    const [pedidosDespachados, setPedidosDespachados] = useState<PedidosAsignadosResponse[]>([]);
    ...
    const fetchPedidosAsignados = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await GetPedidosAsignados("despachado");
            setPedidosDespachados(response || []);
        } catch (err) {
    ...
    return (
        <div>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
                <h1 className="text-2xl font-bold mb-6 text-center">Pedidos Despachados</h1>

                {loading && <p className="text-center text-gray-500">Cargando...</p>}
                {error && <p className="text-center text-red-500 bg-red-100 p-3 rounded-md">{error}</p>}

                {!loading && !error && pedidosDespachados.length === 0 && (
                    <p className="text-center text-gray-600 mt-8">No hay pedidos Despachados.</p>
                )}

                <div className="space-y-8">
                    {pedidosDespachados.map((envio) => (
                        <div key={envio.id_vehiculo} className="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
                            <div className="bg-gray-800 text-white p-4 flex justify-between items-center">
                                ...
                                <button 
                                    className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300 "
                                    disabled
                                >
                                    EN CAMINO
                                </button>
...
export default PedidosDespachadosPage;