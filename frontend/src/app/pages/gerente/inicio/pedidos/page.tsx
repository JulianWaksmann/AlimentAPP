...
import { pedidosEntregados, pedidosEntregadosPorAnio } from "@/app/api/graficos";
...
import { PedidosEntregadosPorMesGrafico } from "@/app/components/graficos/PedidosEntregados";
import Header from "@/app/components/Header";

const PedidosPageGerente = () => {
  const [pedidos, setPedidos] = useState<PedidosEntregados[]>([]);
  const [pedidosPorAnio, setPedidosPorAnio] = useState<PedidosEntregados[]>([]);
  ...
  useEffect(() => {
    async function fetchPedidos() {
      ...
      const dataPedidosPorAnio = await pedidosEntregadosPorAnio(anio);
      setPedidosPorAnio(dataPedidosPorAnio);
    }
    fetchPedidos();
  }, [mes,anio]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="p-6 w-full h-full bg-white-light  border-b border-gray-300 mb-6 ">
        <h2 className="text-2xl font-bold text-primary mb-4 text-center">
          Pedidos entregados por año
        </h2>
        <PedidosEntregadosPorMesGrafico data={pedidosPorAnio} />
      </div>
    </div>
  );
};
