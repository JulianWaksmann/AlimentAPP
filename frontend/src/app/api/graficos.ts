...
export async function pedidosEntregadosPorAnio(anio: number){
      const datos = {
        anio: anio,
    };
    const response = await fetch(`${apiUrl}/reportes-graficos/cant-pedidos-entregados-a-tiempo-y-demora-mensuales`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
